...
- **Branch from an up-to-date default branch** — `git checkout main && git pull` *before* `git checkout -b`. Never cut a branch from another branch whose PR is still open: the child carries the parent's commits, so merging the child lands the parent's work in `main` too, **past the parent PR's own review and CI gate**. This applies to the second and third branch of a session as much as the first — cutting a branch is two steps, not one. Genuinely stacked PRs are a real pattern, but they must be deliberate and called out in the PR body. If you catch a wrong base before pushing, `git rebase --onto main <wrong-base>` fixes it; after it merges, leave it — rewriting shared history costs more than a mislabelled merge commit.
