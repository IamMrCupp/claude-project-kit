...
### Opt-in label scheme (`--with-labels`)

`--with-labels` creates a standard triage label scheme on the current GitHub repo via `gh label create`:

- `type:bug`, `type:feature`, `type:docs`, `type:chore` — mirror the kit's own commit-type / changelog vocabulary
- `priority:P0`, `priority:P1`, `priority:P2`
- `blocker` (gates other work) and `decision-needed` (waiting on a deliberate human call) — pair with the two-tier tracking pattern
- `phase-0`, `phase-1` — lifecycle starters; create further `phase-N` on demand

**Off by default.** This is the *one* structural change the kit will make to a repo — and only on a repo you own, only when you pass the flag. It skips labels that already exist (non-destructive), no-ops cleanly when `gh` isn't installed or there's no GitHub remote, honors `--dry-run` (previews the scheme, creates nothing), and never touches an externally-owned tracker. See `CONVENTIONS.md` → "What the kit does NOT do with trackers" for the rationale behind the carve-out.

```bash
bootstrap.sh ~/Documents/Claude/Projects/foo --with-labels
```
...
**Read-only constraint for tracker *structure*:** the kit never creates tracker projects, workflows, or sprint configurations on your behalf — those are externally-owned across most contexts (work JIRA, team Linear, etc.). Project-level structural artifacts must exist before bootstrap. The one carve-out is **labels on a GitHub repo you own**, via the explicit opt-in `bootstrap.sh --with-labels` (off by default) — see [Opt-in label scheme](#opt-in-label-scheme---with-labels). See [ADR-0001 §D3](docs/adr/0001-multi-repo-folder-model.md) and `CONVENTIONS.md` "Ticket-driven workflows → What the kit does NOT do with trackers".

**Issue-first defaults when you own the tracker:** for personally-owned trackers (your own GitHub repo's Issues, your own Linear team, etc.), the kit's convention shifts: trackable work is mirrored as issues *before* it starts. The bright line is **tracker authority** — `CONTEXT.md`'s Tracker Configuration captures `Tracker authority: user-owned | externally-owned | unknown`, and the convention only applies to user-owned. Mechanics: 1 issue ≈ 1 phase-checklist item, PR body uses `Closes #N`, `phase-N-checklist.md` items record `Issue:` alongside `Branch:` / `PR:`, and bulk-creation always waits for explicit confirmation (single ad-hoc issues during a session don't). [PROMPTS.md Prompt 12](PROMPTS.md#12-carving-a-phase-checklist-into-issues) packages the carve-checklist-into-issues flow. See `CONVENTIONS.md` → "Ticket-driven workflows → Issue-first when you own the tracker" for the full rule.
...
Both feed into **`scripts/upgrade.sh`**, the one-command orchestrator that runs the full kit upgrade flow end-to-end: pulls the kit checkout, runs the sync helpers (auto-detecting workspace mode if applicable), installs any new global helper scripts (under `~/.claude/scripts/`), installs any new global slash commands, prints a clean summary. Refuses to run if the kit checkout is dirty so you don't upgrade against in-flight kit work. Flags: `--dry-run`, `--skip-pull`, `--skip-scripts`, `--skip-commands`, `--force-scripts`, `--force-commands`. See [SETUP.md §Upgrading an existing project](SETUP.md#upgrading-an-existing-project) for the orchestrator's place in the flow plus the manual fallback steps.

**Updating changed kit-shipped commands and scripts.** By default the orchestrator (and `install-commands.sh` + `install-scripts.sh` underneath) is write-once — they add new files without overwriting existing ones. When a kit release ships an *updated* version of a command or helper script you already have installed, pass `--force-commands` and/or `--force-scripts` (orchestrator) or `--force-update --yes` (installers directly). Each overwritten file gets a `.bak.<timestamp>` backup, and files you added that aren't in the kit's templates / scripts stay untouched.
...
Eleven slash commands stage in `<working-folder>/.claude/commands/`. Same install path as agents — `scripts/install-commands.sh --global` (recommended, covers every project) or `--project <repo-path>` (scope to one repo).
...
- **`/archive-ticket <KEY>`** — the resolution counterpart to `/pull-ticket`. Moves a done `tickets/<KEY>-<slug>.md` into `tickets/archive/` (flat) so a long-running workspace's `tickets/` folder stays scannable, then notes the archival in `SESSION-LOG.md`. Never touches the external tracker — transition the ticket there yourself. Terminal equivalent: `archive-ticket.sh <KEY>`.
...
- **`/research <topic-or-area>`** — packages Prompt 9 from `PROMPTS.md`. Deep-reads the codebase area passed as the argument and writes a research artifact in the working folder. Ticket-aware: appends `## Research: <topic>` to the active ticket scratchpad if you're inside one, otherwise writes `research-<slug>.md`. Load-bearing rule: **read + write artifact only, never proposes code changes**. Pairs with `/plan` — research first, then plan against the artifact. Inspired by [Boris Tane's "How I use Claude Code"](https://boristane.com/blog/how-i-use-claude-code/).
- **`/plan <feature>`** — packages Prompt 10 from `PROMPTS.md`. Writes a feature-scoped plan (Goal / Approach / Code snippets / Open questions / numbered todo list at the bottom) to the active ticket scratchpad or `plan-<slug>.md`. Reads any prior `/research` artifact on the same topic before writing. Load-bearing rule: **plan-and-stop, never implements**. Refinement supported via inline annotations on the plan file — re-invoke and the command addresses notes without writing code. Inspired by [Boris Tane's "How I use Claude Code"](https://boristane.com/blog/how-i-use-claude-code/).
...
- **`acme-platform/`** — a fictional AWS Terraform multi-repo workspace driven by JIRA tickets, shaped as a **long-running workspace** hosting multiple initiatives over time. `workspace-CONTEXT.md` (Current Initiative + Past initiatives), `workspace-plan.md` (Active / Planned / Completed roster), `workspace-phase-1-checklist.md` for the active initiative ("Production hardening") + an archived `vpc-foundations-phase-1-checklist.md` from the completed initiative. Per-repo subfolders (`terraform-modules/`, `terraform-envs/`) with their own `CONTEXT.md` / `SESSION-LOG.md`, an active ticket scratchpad ([ACME-1234](examples/acme-platform/tickets/ACME-1234-fix-lb-routing.md)) showing branches/PRs across both repos, and an archived ticket ([ACME-1100](examples/acme-platform/tickets/archive/ACME-1100-add-vpc-module.md)). Best reference for ticket-driven multi-repo work *and* the multi-initiative pattern.
...
- **Doesn't *silently* write to `~/.claude/settings.json`.** That file is global, cross-project, and per-user-per-machine; the kit doesn't sync it from the repo or mutate it without your explicit consent. **One opt-in exception**: `bootstrap.sh --trust-kit-paths` (or the interactive prompt during setup) makes two coordinated additions — appends the working-folder parent to `permissions.additionalDirectories` (silences read-prompts on `CONTEXT.md` / `SESSION-LOG.md`) and appends `Bash(<absolute path to kit-print-memory-pointer.sh>)` to `permissions.allow` (silences the per-session prompt every kit slash command's precheck triggers; absolute path required because the matcher does not expand `~/` or `$HOME` — see [anthropics/claude-code#16800](https://github.com/anthropics/claude-code/issues/16800)). Both writes are idempotent (skip if already present), back up the existing settings.json to `settings.json.bak.<timestamp>` first, and honor `--dry-run`. The kit only mutates those two fields; nothing else in the file is touched. The older `--trust-working-folder-root` flag stays as a deprecation alias with identical behavior. Manual hand-edit stays the alternative for users who'd rather curate the file themselves — see [SETUP.md §1](SETUP.md#1-pick-a-working-folder-location).
...
- **Doesn't manage your tracker / CI structure.** The kit doesn't create tracker projects, workflows, sprint scaffolding, or CI pipelines — those stay user/PM-owned. (One opt-in carve-out: `bootstrap.sh --with-labels` creates a standard label scheme on a GitHub repo *you* own — off by default; see [Opt-in label scheme](#opt-in-label-scheme---with-labels).) Bootstrap-time tracker integration (`/pull-ticket`, `pull-ticket.sh`) is **one-way read** against *any* tracker — fetches summary / AC / status, never creates / edits / transitions / comments. *Work-time* issue creation for **user-owned** trackers is governed by the issue-first convention (Claude proposes the list, user confirms before bulk-create); externally-owned trackers stay read-only. See [ADR-0001 §D3](docs/adr/0001-multi-repo-folder-model.md) and `CONVENTIONS.md` → "Ticket-driven workflows".
