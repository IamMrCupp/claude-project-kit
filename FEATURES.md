...
**Updating changed kit-shipped commands.** By default the orchestrator (and `install-commands.sh` underneath) is write-once — it adds new commands without overwriting existing ones. When a kit release ships an *updated* version of a command you already have installed, pass `--force-commands` (orchestrator) or `--force-update --yes` (install-commands directly). Each overwritten file gets a `.bak.<timestamp>` backup, and files you added that aren't in the kit's templates stay untouched.
...
- **Doesn't *silently* write to `~/.claude/settings.json`.** That file is global, cross-project, and per-user-per-machine; the kit doesn't sync it from the repo or mutate it without your explicit consent. **One opt-in exception**: the working-folder parent directory can be added to `permissions.additionalDirectories` via `bootstrap.sh --trust-working-folder-root` (or the interactive prompt during setup) so Claude Code stops prompting on every read of `CONTEXT.md` / `SESSION-LOG.md`. That single field is the only thing bootstrap touches; the existing file is backed up to `settings.json.bak.<timestamp>` first; the change is idempotent. Manual hand-edit stays the alternative for users who'd rather curate the file themselves — see [SETUP.md §1](SETUP.md#1-pick-a-working-folder-location).
