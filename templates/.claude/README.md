...
- **`/session-verify`** — read-only verification dump: auto-memory dir path, MEMORY.md disk-vs-runtime parity, working-folder + workspace file metadata, with an `OK` / `MISMATCH` / `MISSING` verdict. Use after `/session-start` or when you suspect auto-memory is loading from the wrong path. Same flow as Prompt 13 in `PROMPTS.md`.
...
- **`/archive-ticket <KEY>`** — the resolution counterpart to `/pull-ticket`. Moves a done `tickets/<KEY>-<slug>.md` into `tickets/archive/` (flat) and notes the archival in `SESSION-LOG.md`. Never touches the external tracker. Terminal equivalent: `archive-ticket.sh` at the kit root.
