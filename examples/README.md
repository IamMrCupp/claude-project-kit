...
- `fan-out-walkthrough.md` — not a project snapshot but a **worked pattern walkthrough**: how to use subagent fan-out (the `research-question` agent) on a research-heavy phase, when *not* to, and why the synthesis stays yours. Pairs with `CONVENTIONS.md` → *Subagent fan-out (parallel delegation)*.
